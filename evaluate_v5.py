...
# Number of unique run IDs to process per batch (not CSV rows).
DEFAULT_BATCH_SIZE = 1000
# Max parallel threads; each thread processes one batch at a time.
DEFAULT_MAX_WORKERS = 10
# Cap total run IDs processed. Set to None for all, or an int for pilot runs.
DEFAULT_LIMIT = None
# Flush checkpoint JSON to disk after this many success/failure marks.
CHECKPOINT_FLUSH_INTERVAL = 500
# Max characters allowed per single directory/file name segment.
MAX_PATH_COMPONENT_LENGTH = 200
# Max characters for the entire relative path (all segments combined).
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Only process the first N run IDs (for pilot runs).",
    )
    parser.add_argument(
        "--resume",
...
def main():
    ...
    if args.limit is not None:
        pending_run_ids = pending_run_ids[: args.limit]
        print(f"Pilot mode: limited to {len(pending_run_ids)} run IDs.")
